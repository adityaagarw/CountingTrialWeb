...
const FeedContent = () => {
  ...
  const handleWebSocketMessage = (messageData) => {
      ...
      if (messageData.attribute === 'entry' && messageData.feed_id > 0) {
        console.log('Updating entry count for feed:', messageData.feed_id);
        setActiveFeedContainers((prevContainers) =>
          prevContainers.map((container) =>
            container.feedId === messageData.feed_id
              ? { ...container, entryCount: (container.entryCount || 0) + 1 }
              : container
          )
        );
      } else if (messageData.attribute === 'exit' && messageData.feed_id > 0) {
        console.log('Updating exit count for feed:', messageData.feed_id);
        setActiveFeedContainers((prevContainers) =>
          prevContainers.map((container) =>
            container.feedId === messageData.feed_id
    ...
    try {
      if (feedStartStopStatus[feedId] === 'started') {
        ...
        setActiveFeedContainers((prevContainers) => [
          ...
          {
            feedId,
            //videoSource: `${apiBaseUrl}/feed/view-feed/${feedId}`,
            entryCount: 0,
            exitCount: 0,
          },
        ]);
