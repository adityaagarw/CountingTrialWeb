...
const DashboardContentAnalytics = () => {
  ...
  useEffect(() => {
    allChartData.forEach((chartData, index) => {
      const chartType = chartData[index]?.chart_type || 'line'; // Get the chart type from the first data entry
            ...
            datasets: [
              {
                ...
                tension: chartType === 'line' ? 0.1 : undefined, // Set tension only for line charts
              },
              {
                label: 'Exit',
